...
void NES_mapper248::MemoryWriteSaveRAM(uint32_t addr, uint8_t data)
{
    set_CPU_banks(2 * data, 2 * data + 1, num_8k_ROM_banks - 2, num_8k_ROM_banks - 1);
}

void NES_mapper248::MemoryWrite(uint32_t addr, uint8_t data)
{
    switch (addr & 0xE001)
    {
    case 0x8000:
...
void NES_mapper248::HSync(uint32_t scanline)
{
    if (irq_enabled)
    {
