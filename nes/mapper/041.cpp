...
void NES_mapper41::MemoryWriteSaveRAM(uint32_t addr, uint8_t data)
{
    if (addr < 0x6800)
    {
        regs[0] = (uint8_t)(addr & 0xFF);
...
void NES_mapper41::MemoryWrite(uint32_t addr, uint8_t data)
{
    if (regs[0] & 0x04)
    {
        uint8_t chr_bank = ((regs[0] & 0x18) >> 1) | (data & 0x03);
