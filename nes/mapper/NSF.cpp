...
void NES_mapperNSF::Reset()
{
    ...
    if (bank_switch)
    {
        uint8_t start_bank = ROM_banks[0x9] >> 4;
    ...
    {
        uint32_t nsf_size = ROM_banks[0x0] | (ROM_banks[0x1] << 8) | (ROM_banks[0x2] << 16);
        uint32_t load_addr = ROM_banks[0x8] | (ROM_banks[0x9] << 8);
...
void NES_mapperNSF::MemoryWriteLow(uint32_t addr, uint8_t data)
{
...
void NES_mapperNSF::MemoryWriteSaveRAM(uint32_t addr, uint8_t data)
{
...
void NES_mapperNSF::MemoryWrite(uint32_t addr, uint8_t data)
{
...
uint8_t NES_mapperNSF::MemoryReadLow(uint32_t addr)
{
    return parent_NES->apu->ExRead(addr);
}

void NES_mapperNSF::BankSwitch(uint8_t num, uint8_t bank)
{
    uint32_t load_start = (ROM_banks[0x8] | (ROM_banks[0x9] << 8)) & 0x0FFF;
    if (num == 6 || num == 7)
    {
        for (uint32_t i = 0; i < 0x1000; i++)
        {
            uint32_t adr = 0x1000 * bank + i + 0x80 - load_start;
            if (adr < 0)
            {
