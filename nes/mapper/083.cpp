...
uint8_t NES_mapper83::MemoryReadLow(uint32_t addr)
{
...
void NES_mapper83::MemoryWriteLow(uint32_t addr, uint8_t data)
{
    switch (addr)
    {
    case 0x5101:
...
void NES_mapper83::MemoryWrite(uint32_t addr, uint8_t data)
{
    switch (addr)
    {
    case 0x8000:
    ...
    case 0x8200:
        {
            irq_counter = (irq_counter & 0xFF00) | (uint32_t)data;
        }
        break;

    case 0x8201:
        {
            irq_counter = (irq_counter & 0x00FF) | ((uint32_t)data << 8);
            irq_enabled = data;
        }
        break;
...
void NES_mapper83::HSync(uint32_t scanline)
{
    if (irq_enabled)
    {
        if (irq_counter <= 114)
