...
uint8_t NES_mapper5::MemoryReadLow(uint32_t addr)
{
    uint8_t ret = (uint8_t)(addr >> 8);
    if (addr == 0x5204)
    {
        ret = irq_status;
        irq_status &= ~0x80;
    ...
    {
        ret = (uint8_t)((value0 * value1) & 0x00FF);
    }
    else if (addr == 0x5206)
    {
        ret = (uint8_t)(((value0 * value1) & 0xFF00) >> 8);
    }
    else if (addr >= 0x5C00 && addr <= 0x5FFF)
    {
        if (gfx_mode == 2 || gfx_mode == 3)
        {
            uint8_t* nametable2 = parent_NES->ppu->get_namt() + 0x800;
...
void NES_mapper5::MemoryWriteLow(uint32_t addr, uint8_t data)
{
    uint32_t i;
    ...
    case 0x5106:
        {
            uint8_t* nametable3 = parent_NES->ppu->get_namt() + 0xC00;
            for (i = 0; i < 0x3C0; i++)
            {
                nametable3[i] = data;
            }
...
void NES_mapper5::MemoryWriteSaveRAM(uint32_t addr, uint8_t data)
{
...
void NES_mapper5::MemoryWrite(uint32_t addr, uint8_t data)
{
    if (wram_protect0 == 0x02 && wram_protect1 == 0x01)
    {
...
void NES_mapper5::HSync(uint32_t scanline)
{
    if (scanline <= 240)
    {
...
void NES_mapper5::MMC5_set_CPU_bank(uint8_t page, uint8_t bank)
{
    if (bank & 0x80)
    {
        if (prg_size == 0)
...
void NES_mapper5::MMC5_set_WRAM_bank(uint8_t page, uint8_t bank)
{
    if (bank != 8)
    {
...
uint8_t NES_mapper5::PPU_Latch_RenderScreen(uint8_t mode, uint32_t addr)
{
    uint8_t ex_pal = 0;
    ...
    if (gfx_mode == 1 && mode == 1)
    {
        // ex gfx mode
        uint8_t* nametable2 = parent_NES->ppu->get_namt() + 0x800;
        uint32_t bank = (nametable2[addr] & 0x3F) << 2;
...
void NES_mapper5::sync_Chr_banks(uint8_t mode)
{
    if (chr_size == 0)
    {
