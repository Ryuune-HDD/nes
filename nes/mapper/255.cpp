...
uint8_t NES_mapper255::MemoryReadLow(uint32_t addr)
{
    if (addr >= 0x5800)
    {
...
void NES_mapper255::MemoryWriteLow(uint32_t addr, uint8_t data)
{
    if (addr >= 0x5800)
    {
...
void NES_mapper255::MemoryWrite(uint32_t addr, uint8_t data)
{
    uint8_t prg_bank = (uint8_t)((addr & 0x0F80) >> 7);
    uint8_t chr_bank = (uint8_t)(addr & 0x003F);
    uint8_t rom_bank = (uint8_t)((addr & 0x4000) >> 14);
