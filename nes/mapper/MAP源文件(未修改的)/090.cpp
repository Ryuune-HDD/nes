...
uint8_t NES_mapper90::MemoryReadLow(uint32_t addr)
{
    if (addr == 0x5000)
    {
        return (uint8_t)(value1 * value2 & 0x00FF);
    }
    else
    {
...
void NES_mapper90::MemoryWriteLow(uint32_t addr, uint8_t data)
{
    if (addr == 0x5000)
    {
        value1 = data;
...
void NES_mapper90::MemoryWrite(uint32_t addr, uint8_t data)
{
    switch (addr)
    {
    case 0x8000:
...
void NES_mapper90::HSync(uint32_t scanline)
{
...
void NES_mapper90::Sync_Mirror()
{
    ...
    for (i = 0; i < 4; i++)
    {
        nam_bank[i] = ((uint32_t)nam_high_reg[i] << 8) | (uint32_t)nam_low_reg[i];
    }
...
void NES_mapper90::Sync_Chr_Banks()
{
    ...
    for (i = 0; i < 8; i++)
    {
        chr_bank[i] = ((uint32_t)chr_high_reg[i] << 8) | (uint32_t)chr_low_reg[i];
    }
