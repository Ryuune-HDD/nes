...
void NES_mapper52::MemoryWriteSaveRAM(uint32_t addr, uint8_t data)
{
    if (!regs[1])
    {
        regs[1] = 1;
...
void NES_mapper52::MemoryWrite(uint32_t addr, uint8_t data)
{
    switch (addr & 0xE001)
    {
    case 0x8000:
...
void NES_mapper52::HSync(uint32_t scanline)
{
    if (irq_enabled)
    {
