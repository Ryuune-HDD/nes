...
uint8_t NES_mapper19::MemoryReadLow(uint32_t addr)
{
    if (addr == 0x4800)
    {
        if (patch == 2)
        {
            uint8_t retval = parent_NES->ReadSaveRAM(regs[2] & 0x7F);
    ...
    {
        return (uint8_t)(irq_counter & 0x00FF);
    }
    else if ((addr & 0xF800) == 0x5800) // addr $5800-$5FFF
    {
        return (uint8_t)((irq_counter & 0x7F00) >> 8);
    }
    else
    {
...
void NES_mapper19::MemoryWriteLow(uint32_t addr, uint8_t data)
{
    switch (addr & 0xF800)
    {
    case 0x4800:
    ...
    case 0x5800: // addr $5000-$57FF
        {
            irq_counter = (irq_counter & 0x00FF) | ((uint32_t)(data & 0x7F) << 8);
...
void NES_mapper19::MemoryWrite(uint32_t addr, uint8_t data)
{
    switch (addr & 0xF800)
    {
...
void NES_mapper19::HSync(uint32_t scanline)
{
    if (irq_enabled)
    {
        if (irq_counter >= (uint32_t)(0x7FFF - ((patch == 1) ? 112 : 113)))
        {
            irq_counter = 0x7FFF;
