...
void NES_mapper45::MemoryWriteSaveRAM(uint32_t addr, uint8_t data)
{
    if (addr == 0x6000)
    {
        regs[regs[5]] = data;
...
void NES_mapper45::MemoryWrite(uint32_t addr, uint8_t data)
{
    switch (addr & 0xE001)
    {
    case 0x8000:
...
void NES_mapper45::HSync(uint32_t scanline)
{
    if (irq_enabled)
    {
...
void NES_mapper45::MAP45_set_CPU_bank4(uint8_t data)
{
...
void NES_mapper45::MAP45_set_CPU_bank5(uint8_t data)
{
    data &= (regs[3] & 0x3F) ^ 0xFF;
    data &= 0x3F;
    data |= regs[1];
...
void NES_mapper45::MAP45_set_CPU_bank6(uint8_t data)
{
    data &= (regs[3] & 0x3F) ^ 0xFF;
    data &= 0x3F;
    data |= regs[1];
...
void NES_mapper45::MAP45_set_CPU_bank7(uint8_t data)
{
    data &= (regs[3] & 0x3F) ^ 0xFF;
    data &= 0x3F;
    data |= regs[1];
...
void NES_mapper45::MAP45_set_PPU_banks()
{
    ...
    for (uint8_t i = 0; i < 8; i++)
    {
        ...
        c[i] += (uint32_t)(regs[2] & ((patch == 0) ? 0x10 : 0x30)) << 4;
    }
    if (regs[6] & 0x80)
    {
