...
void NES_mapper47::MemoryWriteSaveRAM(uint32_t addr, uint8_t data)
{
    if (addr == 0x6000)
    {
        if (patch)
...
void NES_mapper47::MemoryWrite(uint32_t addr, uint8_t data)
{
    switch (addr & 0xE001)
    {
    case 0x8000:
...
void NES_mapper47::HSync(uint32_t scanline)
{
    if (irq_enabled)
    {
