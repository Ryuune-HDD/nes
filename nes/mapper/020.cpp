...
void NES_mapper20::Reset()
{
    ...
    {
        uint8_t head1 = fgetc(fp);
        uint8_t head2 = fgetc(fp);
        uint8_t head3 = fgetc(fp);
    ...
    {
        for (uint32_t i = 0; i < ROM_banks[4]; i++)
        {
            for (uint32_t j = 0; j < 65500; j++)
            {
...
uint8_t NES_mapper20::MemoryReadLow(uint32_t addr)
{
    uint8_t val = 0x00;
    ...
    case 0x4032:
        {
            uint8_t eject = ((current_side != 0) && (current_side == last_side)) ? 0 : 1;
            val = 0x40;
            val |= eject ? 1 : 0;
            val |= eject ? 4 : 0;
...
void NES_mapper20::MemoryWriteLow(uint32_t addr, uint8_t data)
{
    switch (addr)
    {
    case 0x4020:
    ...
    case 0x4021:
        {
            irq_latch = (irq_latch & 0x00FF) | ((uint32_t)data << 8);
        }
        break;
...
void NES_mapper20::MemoryWriteSaveRAM(uint32_t addr, uint8_t data)
{
    wram[addr - 0x6000] = data;
}

void NES_mapper20::MemoryWrite(uint32_t addr, uint8_t data)
{
    if (addr < 0xE000)
    {
        wram[addr - 0x6000] = data;
...
void NES_mapper20::HSync(uint32_t scanline)
{
    disk_status &= 0xfc;
