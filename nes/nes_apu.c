...
/* emulation of the 15-bit shift register the ** NES uses to generate pseudo-random series ** for the white noise channel 模拟的15位移位寄存器 NES用来生成伪随机序列 白噪声信道 */
...
/* RECTANGLE WAVE ** ============== ** reg0: 0-3=volume, 4=envelope, 5=hold, 6-7=duty cycle ** reg1: 0-2=sweep shifts, 3=sweep inc/dec, 4-6=sweep length, 7=sweep on ** reg2: 8 bits of freq ** reg3: 0-2=high freq, 7-4=vbl length counter */
...
int apu_rectangle(rectangle_t* chan)
{
    ...
    /* TODO: using a table of max frequencies is not technically ** clean, but it is fast and (or should be) accurate */
...
/* TRIANGLE WAVE ** ============= ** reg0: 7=holdnote, 6-0=linear length counter ** reg2: low 8 bits of frequency ** reg3: 7-3=length counter, 2-0=high 3 bits of frequency */
...
/* WHITE NOISE CHANNEL ** =================== ** reg0: 0-3=volume, 4=envelope, 5=hold ** reg2: 7=small(93 byte) sample,3-0=freq lookup ** reg3: 7-4=vbl length counter */
...
/* DELTA MODULATION CHANNEL ** ========================= ** reg0: 7=irq gen, 6=looping, 3-0=pointer to clock table ** reg1: output dc level, 6 bits unsigned ** reg2: 8 bits of 64-byte aligned address offset : $C000 + (value * 64) ** reg3: length, (value * 16) + 1 */
...
void apu_regwrite(uint32_t address, uint8_t value)
{
    ...
    case APU_WRA0:
    case APU_WRB0:
        /* chan = (address & 4) ? 1 : 0; apu->rectangle[chan].regs[0] = value; apu->rectangle[chan].volume = value & 0x0F; apu->rectangle[chan].env_delay = apu->decay_lut[value & 0x0F]; apu->rectangle[chan].holdnote = (value & 0x20) ? TRUE : FALSE; apu->rectangle[chan].fixed_envelope = (value & 0x10) ? TRUE : FALSE; apu->rectangle[chan].duty_flip = duty_lut[value >> 6]; */
        rect->regs[0] = value;
    ...
    case APU_WRA1:
    case APU_WRB1:
        /* chan = (address & 4) ? 1 : 0; apu->rectangle[chan].regs[1] = value; apu->rectangle[chan].sweep_on = (value & 0x80) ? TRUE : FALSE; apu->rectangle[chan].sweep_shifts = value & 7; apu->rectangle[chan].sweep_delay = apu->decay_lut[(value >> 4) & 7]; apu->rectangle[chan].sweep_inc = (value & 0x08) ? TRUE : FALSE; apu->rectangle[chan].freq_limit = freq_limit[value & 7]; */
        rect->regs[1] = value;
    ...
    case APU_WRA2:
    case APU_WRB2:
        /* chan = (address & 4) ? 1 : 0; apu->rectangle[chan].regs[2] = value; // if (apu->rectangle[chan].enabled) apu->rectangle[chan].freq = (apu->rectangle[chan].freq & ~0xFF) | value; */
        rect->regs[2] = value;
        rect->freq = (apu->rectangle[chan].freq & ~0xFF) | value;
        break;
    case APU_WRA3:
    case APU_WRB3:
        /* chan = (address & 4) ? 1 : 0; apu->rectangle[chan].regs[3] = value; apu->rectangle[chan].vbl_length = apu->vbl_lut[value >> 3]; apu->rectangle[chan].env_vol = 0; apu->rectangle[chan].freq = ((value & 7) << 8) | (apu->rectangle[chan].freq & 0xFF); apu->rectangle[chan].adder = 0; */
        rect->regs[3] = value;
    ...
    /* triangle */
    case APU_WRC0:
        /* apu->triangle.regs[0] = value; apu->triangle.holdnote = (value & 0x80) ? TRUE : FALSE; if (FALSE == apu->triangle.counter_started && apu->triangle.vbl_length) apu->triangle.linear_length = apu->trilength_lut[value & 0x7F]; */
        {
        ...
        break;
    case APU_WRC2:
        /* apu->triangle.regs[1] = value; apu->triangle.freq = APU_TO_FIXED((((apu->triangle.regs[2] & 7) << 8) + value) + 1); */
        {
            triangle_t* tri = &(apu->triangle);
            tri->regs[1] = value;
    ...
    /* noise */
    case APU_WRD0:
        /* apu->noise.regs[0] = value; apu->noise.env_delay = apu->decay_lut[value & 0x0F]; apu->noise.holdnote = (value & 0x20) ? TRUE : FALSE; apu->noise.fixed_envelope = (value & 0x10) ? TRUE : FALSE; apu->noise.volume = value & 0x0F; */
        {
    ...
    /* DMC */
    case APU_WRE0:
        /* apu->dmc.regs[0] = value; apu->dmc.freq = APU_TO_FIXED(dmc_clocks[value & 0x0F]); apu->dmc.looping = (value & 0x40) ? TRUE : FALSE; if (value & 0x80) apu->dmc.irq_gen = TRUE; else { apu->dmc.irq_gen = FALSE; apu->dmc.irq_occurred = FALSE; } */
        {
            dmc_t* dmc = &(apu->dmc);
            dmc->regs[0] = value;
    ...
    case APU_WRE1:
        /* 7-bit DAC */
#if 0
        /* add the _delta_ between written value and ** current output level of the volume reg */
        ...
        break;
    case APU_WRE2:
        /* apu->dmc.regs[2] = value; apu->dmc.cached_addr = 0xC000 + (uint16_t) (value << 6); */
        {
            dmc_t* dmc = &(apu->dmc);
            dmc->regs[2] = value;
        ...
        break;
    case APU_WRE3:
        /* apu->dmc.regs[3] = value; apu->dmc.cached_dmalength = ((value << 4) + 1) << 3; */
        {
            dmc_t* dmc = &(apu->dmc);
            dmc->regs[3] = value;
...
void apu_process(uint16_t* buffer, int num_samples)
{
    ...
    apu->elapsed_cycles = clocks;
    // apu->elapsed_cycles = nes6502_getcycles(0);//*****************************************************************
}
...
void apu_init(void)
{
    ...
    apu_setparams(APU_SAMPLE_RATE, 60, 0, 16); //APU_SAMPLE_RATE为22050Hz
    //采样率
    apu_reset();
