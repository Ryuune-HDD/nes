...
static void am_REL(void)
{
    ...
    PC++; // Advance past operand
    temp16 = target;
}

static void am_ACC(void)
{
    temp16 = 0xDEAD; // Magic number for Accumulator
}
...
{
    // PC already advanced by am_ABS, point to last byte of operand
    PC--;
    push((PC >> 8) & 0xFF);
    push(PC & 0xFF);
    PC = temp16;
...
static void init_op_handlers(void)
{
    ...
    op_handlers[0xEB][1] = op_SBC; // Undocumented
...
void run6502(uint32_t cycles)
{
    ...
    while (clocks < target_cycles)
    {
        ...
        g_page_crossed = 0; // Reset flag

        uint8_t opcode = K6502_Read(PC++);

        op_handlers[opcode][0](); // Addressing Mode
        op_handlers[opcode][1](); // Operation
